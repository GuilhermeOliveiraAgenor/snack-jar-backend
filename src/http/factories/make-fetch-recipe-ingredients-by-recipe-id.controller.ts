...
export function makeFetchRecipeIngredientsByRecipeIdController() {
  ...
  const fetchMyRecipeIngredientsUseCase = new FetchRecipeIngredientsByRecipeIdUseCase(
    recipeIngredientRepository,
    recipeRepository,
  );

  return new FetchRecipeIngredientsByRecipeIdController(fetchMyRecipeIngredientsUseCase);
}
